...
Events follow the DOM capture, target, and bubble phases. `stopPropagation()` prevents the event from reaching later targets while allowing the remaining listeners on the current target to run. `stopImmediatePropagation()` also stops the remaining listeners on the current target.
...
Use a focused `input` or `textarea` when accepting user text. Focusing the element starts the platform text input service, including the on-screen keyboard and IME where the operating system provides them. The committed text is UTF-8 and updates the element's `value` and selection.
...
input.addEventListener('beforeinput', (event) => {
  // Cancel here to reject the edit before value is changed.
...
The runtime exposes these text events:

| Event | Purpose |
|-------|---------|
| `beforeinput` | Cancelable event before committed text changes `value` |
| `input` | Fired after committed text changes `value` |
| `compositionstart` | An IME composition begins |
| `compositionupdate` | The current IME pre-edit text changes |
| `compositionend` | The IME commits or cancels the composition |

`beforeinput` and `input` provide `data`, `inputType`, and `isComposing`. Composition events provide `data`; `start` and `length` expose SDL's current pre-edit selection. Use `element.blur()` to stop native text input.

For a custom rendered editor, position the OS candidate window near the caret:

```javascript
input.setTextInputArea(caretX, caretY, lineHeight, lineHeight, 0);
```

## Clipboard

The system text clipboard is available through the standard promise-based API:
...
Clipboard access is unavailable in `--no-sdl` mode; `writeText()` rejects and `readText()` returns an empty string there.
...
- **`preventDefault()`** cancels the built-in text mutation for `beforeinput`. Other native input events currently have no browser default action to cancel.
- **`stopPropagation()`** and **`stopImmediatePropagation()`** follow DOM propagation semantics across `window`, `document`, and the event target.
...
- **Focus events** are available for runtime-created `input` and `textarea` elements. The canvas itself is still considered focused whenever no text control is active.
